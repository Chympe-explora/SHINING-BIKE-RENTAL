...
(function () {
  ...
  // ---- 🏍️ Bike & Scooter Rental — content + prices (see config.js:
  // window.KC_CONTENT.vehicleRental / rentalPolicy and
  // window.KC_PRICES.vehicleRental) ----
  ...
  // paragraph { text } or a bullet list { type:"list", lead, items }).
  // Shared by the Refund Policy page and the Rental Terms steps so both
  // read from the same renderer instead of two copies of this markup.
  ...
  var Bike = makeIcon([
    ...
    ["path", { d: "M15 6a1 1 0 0 0-1 1v1h3.5l1.5 3" }],
    ["path", { d: "M5.5 17.5 10 8h4" }],
    ["path", { d: "M10 8 13 14h5.5" }],
    ["path", { d: "M12 8h1" }]
  ]);
  var Award = makeIcon([
  ...
  function GlassCard(props) {
    return h(
      "div",
      { id: props.id, className: "kc-glass backdrop-blur-[24px] bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] rounded-[24px] shadow-[0_8px_32px_rgba(0,0,0,0.2),inset_0_1px_0_rgba(255,255,255,0.1)] " + (props.className || "") },
      props.children
    );
  }
  ...
  function App() {
    ...
    // ---- 🏍️ Bike & Scooter Rental form (separate from the camping/trek
    // booking flow above — tapping "Rent Now" on a vehicle card opens
    // this as an overlay on top of whatever page the visitor is on, and
    // "Submit" hands off to WhatsApp exactly like the rest of the site,
    // no backend involved). ----
    var rentalVehicleState = useState(null); var rentalVehicle = rentalVehicleState[0], setRentalVehicle = rentalVehicleState[1];
    var rentalStepState = useState(1); var rentalStep = rentalStepState[0], setRentalStep = rentalStepState[1];
    var rentalFormState = useState({
      plan: "6", pickupDate: "", pickupTime: "", dropDate: "", dropTime: "", destination: "",
      name: "", phone: "", email: "", address: "", license: ""
    });
    var rentalForm = rentalFormState[0], setRentalForm = rentalFormState[1];
    var rentalAgreeState = useState({ terms: false, age: false, fuel: false });
    var rentalAgree = rentalAgreeState[0], setRentalAgree = rentalAgreeState[1];
    var rentalErrorState = useState(""); var rentalError = rentalErrorState[0], setRentalError = rentalErrorState[1];
    var rentalSentState = useState(false); var rentalSent = rentalSentState[0], setRentalSent = rentalSentState[1];

    function openRentalForm(vehicle) {
    ...
    function rentalEstimate() {
      var prices = (rentalVehicle && VEHICLE_RENTAL_PRICES[rentalVehicle.id]) || {};
      return { base: prices[rentalForm.plan] || 0, deposit: prices.deposit || 0 };
    }

    function rentalGoNext() {
      setRentalError("");
      if (rentalStep === 1) {
      ...
      setRentalStep(function (s) { return Math.min(s + 1, 4); });
    }
    function rentalGoBack() {
      setRentalError("");
      if (rentalStep === 1) { closeRentalForm(); return; }
      setRentalStep(function (s) { return Math.max(s - 1, 1); });
    }
    ...
    function submitRentalViaWhatsApp() {
      ...
      var msg =
        ...
        "Plan: " + planLabel + "\n" +
        "Pickup: " + rentalForm.pickupDate + " " + rentalForm.pickupTime + "\n" +
        "Drop-off: " + rentalForm.dropDate + " " + rentalForm.dropTime + "\n" +
        "Destination: " + rentalForm.destination + "\n\n" +
        "Name: " + rentalForm.name + "\n" +
        "Phone: " + rentalForm.phone + "\n" +
        (rentalForm.email ? "Email: " + rentalForm.email + "\n" : "") +
        (rentalForm.address ? "Address: " + rentalForm.address + "\n" : "") +
    ...
    // ---- 🏍️ Rental form modal (4 steps, mirrors the site's normal
    ...
    return h(
      ...
      header, page1, page2, page3, page4, page5, page6, page7, bottomNav, lightbox, rentalFormModal,
